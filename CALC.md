...
> This document describes the mathematical process used to generate the potentiometer model.
> All offline computations are performed in Octave/MATLAB. The C++ runtime only uses precomputed look-up tables (LUT) - ensuring O(1) performance without any floating-point divisions at runtime.

## 1. Physical parameters

Based on the hardware specifications [REQ-PHY-001 to REQ-PHY-003], the ideal sensor has the following constraints:
-  **$V_{ref}$:** 5.0 V
-  **Max Angle:** 355°
...
The sensor is modelled as a strictly linear potentiometer. The conversion from raw ADC counts to physical units is defined as follows:
...
Rather than calculating this at runtime, the Octave script generates a 101 point uniform grid representing 0% to 100% of the potentiometer's travel.
